...
We propose a kernel-friendly edge-aware attention layer for graph generation that is directly compatible with PyTorch FlexAttention, reducing GPU memory by around 65% while substantially accelerating training and sampling without sacrificing generation quality.
